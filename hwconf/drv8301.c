...
static SPIConfig spi_cfg = {
    .end_cb = NULL,
    .ssport = DRV8301_CS_GPIO,
    .sspad = DRV8301_CS_PIN,

    .cr1 = 0xB25, // Corresponds to STM32 HAL settings below
                  //
...
void drv8301_init(void) {
  ...
  // DRV8301 SPI
  palSetPadMode(DRV8301_CS_GPIO, DRV8301_CS_PIN, PAL_MODE_OUTPUT_PUSHPULL);
  ...
  terminal_register_command_callback(
      ...
      "[reg]", terminal_read_reg);

  terminal_register_command_callback("drv8301_write_reg",
                                     "Write to a DRV8301 register.",
                                     "[reg] [hexvalue]", terminal_write_reg);

  terminal_register_command_callback("drv8301_set_oc_adj",
                                     "Set the DRV8301 OC ADJ register.",
                                     "[value]", terminal_set_oc_adj);

  terminal_register_command_callback("drv8301_print_faults",
                                     "Print all current DRV8301 faults.", 0,
                                     terminal_print_faults);
}

/**
 * Set the threshold of the over current protection of the DRV8301. It works by
 * measuring the voltage drop across drain-source of the MOSFETs and activates
 * when it is higher than a set value. Notice that this current limit is not
 * very accurate.
 *
 * @param val
 * The value to use. Range [0 31]. A lower value corresponds to a lower current
 * limit. See the drv8301 datasheet for how to convert these values to currents.
 */
...
static void spi_transfer(uint16_t *in_buf, const uint16_t *out_buf,
                         int length) {
